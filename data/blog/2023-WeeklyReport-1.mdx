...
要部署一个前端项目在nginx上，部署后用域名请求发现请求全部 400，而用IP访问则正常，又是抓包又是看日志排查了好久，其实问题很简单
```
错误的location
location /test {
...
问题： `Redhat Enterprise 8.8` SSH登录时发现执行/etc/profile*时卡住，`Control+C`强制退出初始化过程会导致hostname异常显示，很多env alias等初始化配置都被打断无法正常使用，**但实际业务进程没有问题**
...
反思：后续是不是可以通过 pstack、gdb等工具来排查一下这个死锁问题，又或者几年后灵光一现，对这个问题就有更好解决方法了
...
- 迁移过来发现blog还是不支持以url的方式嵌入youtube bilibili的视频，我还需要去抄一下别人代码
...
- 重写开始看k8s scheduler的源码，特别是informer部分相关的源码
