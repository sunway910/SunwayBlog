...
```bash
docker run --rm -it ghcr.io/vi/websocat wss://testnet-rpc.cess.cloud/ws/
```
...
Currently, we spend about $1,000 on Cloudflare every year, and it will increase in the future. We are willing to pay for quality services, not just to be a freeloader.
