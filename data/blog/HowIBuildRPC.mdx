...
[CF Load Balancing Free Edition 的 websocket 并发限制约为 40000](https://www.sunway.run/blog/CloudflareWSConnectionLimit)，这部分测试结果对不同用户可能有些许差异。如果 `RPC` 服务的并发请求数超过这个限制，`CF Load Balancing` 会返回 `503 Error`，而且不会触发 `Webhook Alert`，所以最好升级 `domain` 至 `pro edition` 甚至 `business edition`。
...
比如将 `rpc pod replicas` 从 2 扩容到 4， 实际上新的 rpc 实例是无法提供正常服务的，它是有状态的，因为新的 `rpc` 实例需要从 0 开始同步区块数据（每个rpc节点的数据有唯一标识，不能直接 copy 其他实例的数据来用）。

当链运行的越久，区块数量越多，需要同步的时间就越多，一般新的实例需要至少等待 2~3 小时才能完成区块同步，之后才能正常提供服务。
